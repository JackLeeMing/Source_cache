...
define(['baiduchart'],function(baiduchart){
    ...
    var flag = null;
    //路面结构，道路等级
    $('#module-daolfb #disTab a').on('click', function() {
        graphtitle = $(this).text();
        var y_type = $('#btns-flag .dropdown-toggle').attr('type');
        var x_type = $(this).attr("name");

        $.get("/manage/road/area/count", {'y_type' : y_type, 'x_type' : x_type}, function(rs) {
            if (rs.data) {
                flag == 1;
                RS = rs;
    ...
    //管理单位，地区
    $('#module-daolfb #btns-flag li').on("click", function() {
    ...
    $('#module-daolfb .table-export').on('click', function() {
        var y_type = $('#btns-flag .dropdown-toggle').attr('type');
        var x_type = $('#disTab').find("li.active").attr("name");
        location.href = "/manage/road/area/count?y_type=" + y_type + '&x_type=' + x_type + "&export=1";
    });

    //th
    $('body').on('click','#module-daolfb .gx',function(){
    ...
    //td
    $('body').on('click','#module-daolfb .gy',function(){
        graphtitle = $(this).text() +'(' + $.trim($('#disTab').find(".active").text()) + ')';
        var y_type = $('#btns-flag .dropdown-toggle').attr('type');
        var x_type = $('#disTab').find(".active").attr("name");
    ...
    var drawgraph = function(graphdata){
        ...
        $('#module-daolfb #daolfb-chart-text').empty();
